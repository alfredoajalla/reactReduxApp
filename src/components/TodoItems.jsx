  ...
  return (
    <li className="flex flex-col sm:flex-row sm: items-center justify-between border-b-2 py-2 gap-4">
      <div className="flex items-center">
        ...
        <span className={`mr-4 ${todo.completed ? "line-througth text-red-500" : ""}`}>{todo.text}</span>
      </div>
      <div className="space-x-3 ml-8">
        <button onClick={() => dispatch(toggleTodo(index))} className="mr-2 text-sm bg-blue-500 text-white sm:px-2 py-1 rounded ">{todo.completed ? <FaToggleOff /> : <FaToggleOn />}</button>
        ...
        {
          todo.completed && (
            <button onClick={() => dispatch(markIncompleted(index))} className="mr-2 text-sm bg-yellow-500 text-white sm:px-2 py-1 rounded "><FaTimes /></button>
          )
        }
      </div>
    </li >
